...
$(document).on('ready', function () {
    ...
    if (user.username!=='') {
        console.log(`The user ${user.username} is ${user} `);
        $('#createLink').show();
        $('#tradeLink').show();
        $('#holdingLink').show();
    ...
    $('.sort-selector').click(function () {
        ...
        let arrow_icon = (sort_reversed ? 'fa-arrow-up' : 'fa-arrow-down');
    ...
    function onMessage(msg) {
        try {
            ...
            if (data.msg === 'papers') {
                ...
                let e = formatDate(data.blockstats.transactions[0].timestamp.seconds * 1000, '%M/%d/%Y &nbsp;%I:%m%P');
 ...
 * @param {Object[]} papers The list of trades to display.
...
function build_trades(papers, panelDesc) {
    ...
    if (papers && papers.length > 0) {
        ...
        for (let paper in papers) {
            let broken_up = paper_to_entries(papers[paper]);
        ...
        if (rows.length == 0) {
            ...
            let tableBody = $(panelDesc.tableID);
            tableBody.empty();
...
function processFilterForm(panelDesc) {
    ...
    let form = document.forms[panelDesc.formID];
    ...
    for (let i in names) {
        ...
        let name = names[i];
        let id = panelDesc.filterPrefix + name;
